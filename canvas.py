...
dap_connection_string = os.environ.get("DAP_CONNECTION_STRING")
namespace = 'canvas'

async def main():
    credentials = Credentials.create(client_id=dap_client_id, client_secret=dap_client_secret)
    tables = await async_get_tables(api_url, credentials, namespace)

    for t in tables:
        result = await async_sync_table(t, api_url, credentials, namespace, dap_connection_string)
        if result == SyncTableResult.INIT_NEEDED:
            result == await async_init_table(t, api_url, credentials, namespace, dap_connection_string)
...
async def async_sync_table(table: str, api_url: str, credentials: Credentials, namespace:str, connection_string:str):
...
async def async_init_table(table: str, api_url: str, credentials: Credentials, namespace:str, connection_string:str):
